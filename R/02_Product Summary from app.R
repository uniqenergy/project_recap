...
drive<-'G:/My Drive/$Technical$ New/Technical/R_Scripts/01_Projects/Shiny/DEVON/data' #paste drive location here and change backslashes
directory<-'100_04-25-064-07W400 Devon 4B HZ WOLF' #paste well location
########################################################################
...
setwd(wd)
#doc<- xmlTreeParse(file = 'ETS_0Y0T_0439_2018_MM_DD_1.xml', useInternal = TRUE)
doc<- xmlTreeParse(file = paste0(xml_path), useInternal = TRUE)
rootNode<-xmlRoot(doc)

well_data<-well_info() # well info from function
UWI<- read.xlsx('Inventory.xlsx', 
                sheetName='Well Export', header = TRUE,
                colClasses = NA, stringsAsFactors=FALSE)
UWI<-as.data.table(UWI)
UWI<-UWI[2,]
setnames(UWI, c('Title','UWI'))
UWI<-UWI$UWI
...
usage<-usage[,!c('TotalUsed','TotalCost')]
usage[, Price...:=as.numeric(Price...)]
usage<-melt(usage, id.vars = c('ProductName','Unit','Price...'))
usage[, value:=as.numeric(value)]
setnames(usage, old=c('variable','value','Price...'), new=c('report','quantity','Unit_Price'))
...
usage<- usage[!ProductName %in% c('Pallets', 'Pallet','Drums', 'pallet', 'pallets','Van Rental','Trucking','Corrosion Rings', 'Enviro bags','Enviro Bags'),]
usage[, ProductName:=gsub("  "," ",ProductName)]
usage[, UWI:=UWI]

total<-usage[, list(total=sum(quantity, na.rm=TRUE)), by=c('UWI','ProductName', 'Unit','Unit_Price')]
...
total[, ProductName:=gsub('247rd','247RD', ProductName)]
...
total[, ProductName:=gsub('HVd','HVD', ProductName)]
total[, ProductName:=gsub('Hd','HD', ProductName)]
total[, ProductName:=gsub('A1703d','A1703D', ProductName)]
total[, ProductName:=gsub('1103d','1103D', ProductName)]
...
setorderv(total2, c('Cost'), order = -1)

#table2_______________________________________________________________________
#tables
my_table_2<-regulartable(total2[,c('Product_Type','Cost','Percent_of_total')])
...
#apply helper functions
my_table_2<-set_formatter(my_table_2, Cost=currency_format)
...
my_table_2<-set_formatter(my_table_2, Percent_of_total=percent_format)
...
pie<- ggplot(total) + 
  ...
  #"/n **Base oil quantities have been split 50% with b-79-H_94-B-09')+
